...
const Collections = () => {
  const { getAllCollection, collections, handleDeleCollection } = useContext(Context)
  ...
  useEffect(() => {
    setData(collections?.slice((page - 1) * resultsPerPage, page * resultsPerPage));
  }, [page, resultsPerPage, collections]);
  ...
  return (
    <div>
      ...
      <div className="flex text-gray-800 dark:text-gray-300">
        ...
        {">"}
        <p className="mx-2">All Collections</p>
      </div>
            ...
            <TableBody>
              {data?.map((collection) => (
                <TableRow key={collection?.id}>
                  <TableCell>
                    <div className="flex items-center text-sm">
                      <Avatar
                        ...
                        src={baseURLImg + collection?.attributes?.collectionImg?.data[0].attributes?.url}
                        alt="Product image"
                      />
                      <div>
                        <p className="font-semibold">{collection?.attributes?.collectionName}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-sm">{collection?.attributes?.collectionDesc.slice(0, 60)} ...</TableCell>
                  <TableCell>
                    <div className="flex">
                      <Link to={`/app/edit-collection/${collection?.id}`}>
                        <Button
                          icon={EditIcon}
                          className="mr-3"
                          layout="outline"
