...
const AddProduct = () => {
  ...
  const { getAllCategories, categories, getAllBrands, brands, getAllCollection, collections, handleSubmit } = useContext(Context);
  ...
  const handleNewProduct = () => {
    const data = {
      ...
      productPrice: Number(updatesData.productPrice),
      productQuantity: Number(updatesData.productQuantity),
      productDesc: updatesData.productDesc,
      productDiameter: Number(updatesData.productDiameter),
      productHeart: 0,
      productStar: 0,
      ...
      <div className="w-full mt-8 grid gap-4 grid-col md:grid-cols-3 ">
        <Card className="row-span-2 md:col-span-2">
          <CardBody>
            ...
            {updatesData.productName && (
              <div className="border-2 border-dashed rounded-lg p-3 h-48 mb-3 flex items-center">
                ...
                <div className="w-2/3 font-medium py-3">
                  ...
                  <div className="flex my-3">
                    <label className="w-1/2"><span>Brand: {inputBrand}</span></label>
            ...
            <div className="w-full" onClick={() => handleNewProduct()}>
          ...
          <CardBody>
            ...
            <div class="form form-media mb-3">
              ...
              <div class="media flex justify-center ">
                ...
                <div class="media-img w-1/2 ml-3">
                  {imageUrl && <img src={imageUrl} alt="Selected Image" class="w-auto h-24" />}
                </div>
              </div>
            </div>
            ...
            <Label className="mt-4 border-b pb-6 border-gray-500">
              ...
              >
                ...
                {categories?.map((category) => (
                  <option key={category.id} value={category?.id}>{category?.attributes?.categoryName}</option>
                ))}
              </Select>
            </Label>
            ...
            <Label className="mt-4 border-b pb-6 border-gray-500">
              ...
              <Select className="mt-1"
                ...
                {brands?.map((brand) => (
                  <option key={brand.id} value={brand?.id}>{brand.attributes.brandName}</option>
                ))}
              </Select>
            </Label>
            ...
            <Label className="mt-4">
              ...
              >
                ...
                {collections?.map((collection) => (
                  <option key={collection.id} value={collection?.id}>{collection.attributes.collectionName}</option>
                ))}
              </Select>
            </Label>
          </CardBody>
